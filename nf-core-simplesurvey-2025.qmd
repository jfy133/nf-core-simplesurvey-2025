...
## Concordence between two manual reviewers

To roughly compare how the two reviewers tagged each response, we can do cross-line bars of the rank ordering of the most to least commonly found tags across all responses.

```{r}
#| fig-width: 12
#| fig-height: 8

plot_tags_comparison <- function(r1, r2, question_type) {

  data_comparison_positive <- full_join(
      r1 |> rowid_to_column() |> select(-count),
      r2 |> rowid_to_column() |> select(-count),
      by = 'topic'
    ) |>
    rename(r1_rank = rowid.x, r2_rank = rowid.y) |>
    select(topic, r1_rank, r2_rank) |>
    pivot_longer(cols = !topic,
                 names_to = 'reviewer',
                 values_to = 'rank')

  ggplot(data_comparison_positive,
         aes(x = reviewer, y = rank, group = topic)) +
    scale_y_reverse() +
    geom_point() +
    geom_line(colour = "#24b064") +
    theme_minimal() +
    geom_label(aes(label = topic)) +
    labs(title = paste("Comparison of tags for question type:", question_type))
}

plot_tags_comparison(data_tags_positive, data_tags2_positive, "positive")
plot_tags_comparison(data_tags_improve, data_tags2_improve, "improvement")
plot_tags_comparison(data_tags_request, data_tags2_request, "requests")
  
```

This image is not so useful (I tried to do a alluvium plot with `ggaluvium` but could not work out to get it to work sadly).
